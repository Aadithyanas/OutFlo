...
class LinkedInScraperService {
  constructor() {
    this.email = config.linkedinCredentials.email;
    this.password = config.linkedinCredentials.password;
  ...
  async getConnections(maxConnections = null) {
    if (!this.driver) {
    ...
    if (!loggedIn) return { success: false, message: "Failed to login" };
  ...
  async getDetailedProfileData(urls) {
    if (!this.driver) {
      const initialized = await this.initialize();
      if (!initialized) return { success: false, message: "Failed to initialize driver" };
    }
  ...
  async exportToJson() {
    try {
      const connections = await Connection.find({}, { _id: 0 }).lean();
